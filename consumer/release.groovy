...
def projectProps = new File('conanfile.py').text
def versionMatch = projectProps =~ /(?m)version\s*=\s*"(.*)"/
def version = versionMatch[0][1]

def prevTag = 'git describe --abbrev=0  --tags --match=pact-cpp-consumer-*'.execute().text.trim()
def changelog = []
...
ask('Tag and Push commits?: [Y]') {
  ...
  executeOnShell("git tag pact-cpp-consumer-v${releaseVer}")
  executeOnShell 'git push --tags'
}

def nextVer = Version.valueOf(releaseVer).incrementPatchVersion()
ask("Bump version to $nextVer?: [Y]") {
  executeOnShell "sed -i -e 's/project(pact-cpp-consumer VERSION ${releaseVer})/project(pact-cpp-consumer VERSION ${nextVer})/' CMakeLists.txt"
  executeOnShell "sed -i -e 's/version = \"${releaseVer}\"/version = \"${nextVer}\"/' conanfile.py"
  executeOnShell "sed -i -e 's/PROJECT_NUMBER         = \"${releaseVer}\"/PROJECT_NUMBER         = \"${nextVer}\"/' Doxyfile"
  executeOnShell("git add CMakeLists.txt conanfile.py Doxyfile")
